...
def train(epoch, logger):
    epoch_loss = 0
    begin = time.time()
    batch_num = int(np.floor(opt.epoch_sam_num / opt.batch_size))   #5k/5
    for i in range(batch_num):
        gt_batch = shuffle_crop(train_set, opt.batch_size)  #输入的measurement打乱，一些预处理
        gt = Variable(gt_batch).cuda().float()  #为了算sssim等参数就得这样，变量不一定参与网络训练（网络中的参数才会变化）
        input_meas = init_meas(gt, mask3d_batch_train, opt.input_setting)  #反转色散，的初始化信号h
        # stx()
        optimizer.zero_grad()
